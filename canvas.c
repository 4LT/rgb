...
int draw(size_t width, size_t height, pixel_t *pixmap)
{
    ...
    const uint32_t R_MASK = 0xFF0000;
    const uint32_t G_MASK = 0xFF00;
    const uint32_t B_MASK = 0xFF;
    ...
    win = SDL_CreateWindow("KD Tree", SDL_WINDOWPOS_UNDEFINED,
    ...
    SDL_SetSurfaceBlendMode(inputSurf, SDL_BLENDMODE_NONE);
    if (inputSurf == NULL)
        return sdlErr();
