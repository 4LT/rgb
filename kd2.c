...
int red(pixel_t pix) { return (pix >> 16) & 0xFF; }
int green(pixel_t pix) { return (pix >> 8) & 0xFF; }
int blue(pixel_t pix) { return pix & 0xFF; }
...
bool unsaturated(pixel_t pix)
{
    return red(pix) == blue(pix) && blue(pix) == green(pix);
}
...
{
    bool lt;
    if (unsaturated(pix1) || unsaturated(pix2)) {
        lt = interleave_rgb(pix1) < interleave_rgb(pix2);
    }
    else {
        if (hue(pix1) < hue(pix2))
            lt = true;
        else if (hue(pix1) > hue(pix2))
            lt = false;
        else
            lt = interleave_rgb(pix1) < interleave_rgb(pix2);
    }
